...
export function ExperimentModal({ open, defaultProjectId = DEFAULT_PROJECT_ID, onClose, onCreated }: ExperimentModalProps) {
  ...
  const activeProject: DemoProject = useMemo(() => {
    ...
    return {
      ...
      objective: newProject.projectObjective || 'Local project created from the New Experiment workflow.',
      techniques: techniqueScope,
      techniqueMetadata: techniqueScope.map((item) => ({
        key: item,
        label: item,
        role: item === technique ? 'primary' : 'supporting',
        status: item === technique ? 'ready' : 'pending',
        dataAvailable: item === technique,
      })),
      evidenceSources: [],
      ...
      validationGaps: [
        {
          id: `${localProjectId}-initial-validation`,
          description: 'Process experimental evidence before making a material claim.',
          severity: 'moderate',
          suggestedResolution: 'Run the selected technique workspace and review extracted evidence.',
        },
      ],
      nextDecisions: [],
